...
@click.pass_context
def cli(ctx: click.Context, config: Optional[str], profile: Optional[str], verbose: bool):
    ...
    if verbose:
        ...
        click.echo(f"⚙️  Config validated and loaded: {roma_config.app.name} v{roma_config.app.version}")
...
@click.pass_context
def execute(ctx: click.Context, goal: str, profile: Optional[str], hitl: bool, stream: bool, max_steps: int):
...
@click.option('--hitl/--no-hitl', default=False, help='Enable Human-in-the-Loop')
@click.pass_context
def research(ctx: click.Context, topic: str, profile: str, hitl: bool):
...
@click.pass_context
def async_execute(ctx: click.Context, goal: str, max_steps: int, save_state: bool):
