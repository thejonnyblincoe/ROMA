...
class ModelFactory:
    ...
    def _get_api_key_for_model(self, model_id: str) -> tuple[str | None, str | None]:
        """
        ...
        for provider, env_key in self._provider_env_keys.items():
            if model_id.startswith(f"{provider}/") or (
                provider == "openai" and "/" not in model_id
            ):
                if env_key:
                    ...
                    if api_key:
                        ...
                        logger.debug(
                            f"API key required but not found for provider: {provider} (env var: {env_key})"
                        )
                        return None, env_key
                else:
