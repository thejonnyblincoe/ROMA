...
from sqlalchemy import String, DateTime, Integer, Index, Text, Enum as SQLEnum, ForeignKey
...
class TaskExecutionModel(Base):
    ...
    task_type: Mapped[str] = mapped_column(
        ...
        nullable=False,
        index=True,
        comment="Task type (RETRIEVE, WRITE, THINK, etc.)"
    )

    node_type: Mapped[Optional[str]] = mapped_column(
        String(50),
        nullable=True,
        comment="Node type (PLAN, EXECUTE)"
    )
    ...
    root_task_id: Mapped[Optional[str]] = mapped_column(
        ...
        nullable=True,
        index=True,
        comment="Root task ID for the entire execution tree"
    )
    ...
    depth_level: Mapped[int] = mapped_column(
        ...
        nullable=False,
        default=0,
        comment="Depth in the task hierarchy (0 = root)"
    )
    ...
    task_metadata: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Task metadata and configuration"
    )
    ...
    agent_config: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Agent configuration used for execution"
    )

    execution_context: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Execution context and environment"
    )
    ...
    cost_info: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Cost information"
    )
    ...
    is_archived: Mapped[bool] = mapped_column(
        ...
        default=False,
        index=True,
        comment="Whether execution has been archived"
    )
...
class TaskRelationshipModel(Base):
    ...
    order_index: Mapped[Optional[int]] = mapped_column(
        Integer,
        nullable=True,
        comment="Order index for ordered relationships"
    )

    relationship_metadata: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        comment="Relationship-specific metadata"
    )
