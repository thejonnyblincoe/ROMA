...
class ExecutionCheckpointModel(Base):
    ...
    execution_id: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True, comment="Execution ID this checkpoint belongs to"
    )

    checkpoint_name: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Human-readable checkpoint name"
    )
    ...
    checkpoint_data: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, comment="Serialized execution state data"
    )

    # Large binary data (for complex state)
    large_data: Mapped[bytes | None] = mapped_column(
        LargeBinary, nullable=True, comment="Large binary checkpoint data"
    )

    # Checkpoint metadata
    task_graph_snapshot: Mapped[dict[str, Any] | None] = mapped_column(
        JSONB, nullable=True, comment="Task graph state at checkpoint time"
    )

    execution_context: Mapped[dict[str, Any] | None] = mapped_column(
        JSONB, nullable=True, comment="Execution context at checkpoint time"
    )

    agent_states: Mapped[dict[str, Any] | None] = mapped_column(
        JSONB, nullable=True, comment="Agent states at checkpoint time"
    )

    # Checkpoint versioning
    version: Mapped[int] = mapped_column(
        Integer, nullable=False, default=1, comment="Checkpoint format version"
    )

    sequence_number: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="Sequence number within execution"
    )

    # Recovery information
    recovery_instructions: Mapped[dict[str, Any] | None] = mapped_column(
        JSONB, nullable=True, comment="Instructions for recovery from this checkpoint"
    )

    dependencies: Mapped[dict[str, Any] | None] = mapped_column(
        JSONB, nullable=True, comment="Dependencies required for recovery"
    )

    # Checkpoint validity
    expires_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, index=True, comment="When this checkpoint expires"
    )

    is_valid: Mapped[bool] = mapped_column(
        nullable=False, default=True, index=True, comment="Whether checkpoint is still valid"
    )

    # Performance metadata
    data_size_bytes: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Size of checkpoint data in bytes"
    )

    creation_duration_ms: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Time taken to create checkpoint in milliseconds"
    )
...
class RecoveryStateModel(Base):
    ...
    task_id: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True, comment="Task ID being recovered"
    )

    execution_id: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True, comment="Execution ID being recovered"
    )

    checkpoint_id: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="Checkpoint used for recovery"
    )
    ...
    state_data: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, comment="Recovery state data"
    )

    error_context: Mapped[dict[str, Any] | None] = mapped_column(
        JSONB, nullable=True, comment="Error context that triggered recovery"
    )

    recovery_strategy: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="Recovery strategy used"
    )
    ...
    recovery_completed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="When recovery completed"
    )

    recovery_duration_ms: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Recovery duration in milliseconds"
    )

    # Recovery outcome
    success: Mapped[bool | None] = mapped_column(
        nullable=True, comment="Whether recovery was successful"
    )

    recovery_result: Mapped[dict[str, Any] | None] = mapped_column(
        JSONB, nullable=True, comment="Recovery operation result"
    )

    failure_reason: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Reason for recovery failure"
    )

    # Recovery versioning
    version: Mapped[int] = mapped_column(
        Integer, nullable=False, default=1, comment="Recovery state format version"
    )

    attempt_number: Mapped[int] = mapped_column(
        Integer, nullable=False, default=1, comment="Recovery attempt number"
    )
