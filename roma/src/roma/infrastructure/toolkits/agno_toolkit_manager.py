...
from .base_agno_toolkit import BaseAgnoToolkit, DefaultAgnoToolkitWrapper

logger = logging.getLogger(__name__)


class AgnoToolkitManager(IToolkitManager):
    """
    ...
    async def read_toolkit(self, toolkit_name: str) -> Any | None:
        """
    ...
    async def list_default_agno_toolkits(self) -> list[dict[str, Any]]:
        ...
        return [
            {"name": name, "type": toolkit.toolkit_type, "source": "agno_default"}
    ...
    async def find_toolkits_by_capability(
        self, required_tools: list[str], preferred_types: list[str] | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    async def get_registry_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "availability_percentage": round(
                (enabled_count / total_toolkits * 100) if total_toolkits > 0 else 0, 2
            ),
    ...
    async def get_available_tools(self) -> list[Any]:
        ...
        logger.debug(
            f"Found {len(available_tools)} total tools from {len(self._toolkits)} toolkits"
        )
