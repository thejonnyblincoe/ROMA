...
from roma.infrastructure.utils.async_file_utils import (
    async_calculate_directory_size,
    async_cleanup_old_files,
    async_copy2,
    async_file_size,
    async_glob,
    async_is_dir,
...
class LocalFileStorage(IStorage):
    ...
    def __init__(self, config: StorageConfig, execution_id: str | None = None):
    ...
    async def initialize(self) -> None:
        ...
        if self.config.create_subdirs:
            await async_makedirs(
                self.mount_path / self.config.artifacts_subdir, parents=True, exist_ok=True
            )
            await async_makedirs(
                self.mount_path / self.config.temp_subdir, parents=True, exist_ok=True
            )
            await async_makedirs(
                self.mount_path / self.config.results_subdir, parents=True, exist_ok=True
            )
            await async_makedirs(
                self.mount_path / self.config.plots_subdir, parents=True, exist_ok=True
            )
            await async_makedirs(
                self.mount_path / self.config.reports_subdir, parents=True, exist_ok=True
            )
            await async_makedirs(
                self.mount_path / self.config.logs_subdir, parents=True, exist_ok=True
            )
    ...
    async def put_text(self, key: str, text: str, encoding: str = "utf-8", metadata: dict[str, str] | None = None) -> str:
        """
        Store text content.
    ...
    async def cleanup_temp_files(self, older_than_hours: int = 24) -> int:
        ...
        try:
            cleaned_count = await async_cleanup_old_files(temp_path, older_than_hours, "*")
