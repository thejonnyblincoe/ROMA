...
class AgnoFrameworkAdapter(FrameworkAdapter):
    ...
    async def run(
        ...
        **kwargs,
        ...
        agent = Agent(name=agent_name, model=model, output_schema=output_schema)

        # Add tools if specified
        if tools and self.toolkit_manager:
            for tool_name in tools:
                try:
                    toolkit_spec = {"name": tool_name, "type": "default", "enabled": True}
        ...
        try:
            ...
            if is_test_env:
                ...
                if "search" in task.goal.lower():
                    result_content = (
                        "Found 3 research papers on AI: [Paper 1], [Paper 2], [Paper 3]"
                    )
