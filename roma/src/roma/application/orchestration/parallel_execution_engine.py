...
class ParallelExecutionEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"ParallelExecutionEngine initialized with max_concurrent_tasks={max_concurrent_tasks}"
        )
    ...
    async def execute_ready_nodes(
        ...
        logger.info(
            f"Executing {len(ready_nodes)} nodes in parallel (max concurrent: {self.max_concurrent_tasks})"
        )

        # Pre-execution dependency validation
        executable_nodes = await self.dependency_validator.get_executable_nodes(
            ready_nodes, self.state_manager.graph
        )

        if len(executable_nodes) < len(ready_nodes):
            skipped_count = len(ready_nodes) - len(executable_nodes)
            logger.warning(
                f"Dependency validation filtered out {skipped_count} nodes from execution"
            )
        ...
        for node in executable_nodes:
            task = self._execute_single_node_with_semaphore(
                node, agent_service_registry, context, execution_state
            )
        ...
        async with self._execution_semaphore:
            ...
            try:
                ...
                if current_status == TaskStatus.PENDING:
                    ...
                    logger.debug(
                        f"Processing WAITING_FOR_CHILDREN node {node_id} without state change"
                    )
                    ...
                    logger.warning(
                        f"Node {node_id} has unexpected status {current_status} for execution"
                    )
                    # Try to process anyway, but this might fail

                logger.debug(f"Processing node {node_id} through agent pipeline")

                # Build per-node enriched context using context builder
                enriched_context = base_context
                if self.context_builder:
                    try:
                        enriched_context = await self.context_builder.build_per_node_context(
                            node, base_context, execution_state
                        )
                    except Exception as e:
                        logger.warning(
                            f"Failed to build per-node context for {node.task_id}, using base context: {e}"
                        )

                # Process node through agent services
                result = await self._process_node_with_agent_services(
                    node, agent_service_registry, enriched_context, execution_state
                )
                ...
                try:
                    if (
                        ...
                        or ("node_id" not in result.metadata)
                    ):
    ...
    async def _process_node_with_agent_services(
        ...
        try:
            ...
            # Context is already enriched when passed to this method
            ...
            if node.status == TaskStatus.NEEDS_REPLAN:
                ...
                logger.info(
                    f"Replanning {node.task_id}: {len(failed_children)} failed, "
                    f"{len(completed_children)} completed"
                )

                # Route to plan modifier with context
            ...
            atomizer_result = await atomizer_service.run(
                task=node, context=context, execution_id=execution_state.execution_id
            )
            ...
            if atomizer_result.action == NodeAction.NOOP:
                ...
                if atomizer_result.metadata and "node_type" in atomizer_result.metadata:
                    ...
                    if hasattr(atomizer_response, "is_atomic"):
                        node_type = (
                            NodeType.EXECUTE if atomizer_response.is_atomic else NodeType.PLAN
                        )
                    else:
