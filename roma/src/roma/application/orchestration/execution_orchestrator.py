...
class ExecutionOrchestrator:
    ...
    async def execute(
        self, root_task: TaskNode, overall_objective: str, execution_id: str
    ) -> ExecutionResult:
        """
        ...
        self.execution_state = ExecutionState(
            execution_id=execution_id, overall_objective=overall_objective, root_task=root_task
        )

        # Create task node processor for this execution's state management
        processor = TaskNodeProcessor(
            graph_manager=self.graph_state_manager, execution_state=self.execution_state
        )

        logger.info(
            f"Starting execution orchestration [{execution_id}] for root task: {root_task.task_id}"
        )
        ...
        try:
            ...
            if self.graph_state_manager.has_cycles():
                raise ValueError(
                    f"Dependency cycle detected in task graph for execution {execution_id}. Cannot execute."
                )
            ...
            while not self._is_execution_complete():
                ...
                if state.iterations >= self.execution_config.max_iterations:
                    logger.warning(
                        f"Execution stopped: max iterations ({self.execution_config.max_iterations}) reached"
                    )
                    break
                ...
                if self._is_timeout_exceeded(self.execution_state):
                    logger.warning(
                        f"Execution stopped: timeout ({self.execution_config.total_timeout}s) exceeded"
                    )
                    break
                ...
                if not ready_nodes:
                    ...
                    if deadlock_reports:
                        ...
                        executing_nodes = [
                            n
                            for n in self.graph_state_manager.get_all_nodes()
                        ...
                        if executing_nodes:
                            ...
                            logger.debug(
                                f"Waiting for {len(executing_nodes)} executing nodes to complete"
                            )
                ...
                if len(ready_nodes) > self.execution_config.max_tasks_per_level:
                    ...
                    logger.warning(
                        f"Limited ready nodes to {self.execution_config.max_tasks_per_level}"
                    )

                # Execute ready nodes in parallel
                logger.info(
                    f"Iteration {state.iterations + 1}: Processing {len(ready_nodes)} ready nodes"
                )
            ...
            return await self._create_execution_result(
                root_task.task_id, execution_id, error=str(e)
            )
        finally:
    ...
    async def _handle_node_results_with_processor(
        self, node_results: list[NodeResult], processor: TaskNodeProcessor
    ) -> None:
        """
    ...
    async def _handle_orchestration_logic(self, result: NodeResult) -> None:
        ...
        if (result.action in [NodeAction.COMPLETE, NodeAction.AGGREGATE]
            and result.envelope and self.knowledge_store):
                try:
                    node = self.graph_state_manager.get_node_by_id(result.task_id)
                    if node:
                        record = await self.knowledge_store.add_or_update_record(
                            node, result.envelope
                        )
                        logger.debug(
                            f"Persisted node {result.task_id} result to KnowledgeStore: {record.task_id}"
                        )
                except Exception as e:
    ...
    async def _persist_execution_history(self, result: NodeResult) -> None:
        ...
        try:
            ...
            if result.envelope:
                ...
                if result.envelope.execution_metrics:
                    ...
                    execution_duration_ms = int(
                        result.envelope.execution_metrics.execution_time * 1000
                    )
            ...
            logger.debug(
                f"Persisted execution history for task {result.task_id} with status {status}"
            )

        except Exception as e:
            logger.error(f"Failed to persist execution history for task {result.task_id}: {e}")

    async def _get_cached_result(self, node_id: str) -> AnyResultEnvelope | None:
    ...
    def _is_execution_complete(self) -> bool:
        ...
        return all(node.status in {TaskStatus.COMPLETED, TaskStatus.FAILED} for node in all_nodes)
    ...
    async def _create_execution_result(
        self, root_task_id: str, execution_id: str, error: str | None = None
    ) -> ExecutionResult:
        ...
        if not self.execution_state:
            ...
            return ExecutionResult(
                ...
                error_details=[
                    {
                        "type": "execution_state_error",
                        "message": f"No state found for {execution_id}",
                    }
                ],
            )
