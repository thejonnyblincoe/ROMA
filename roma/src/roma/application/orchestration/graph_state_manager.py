...
class GraphStateManager(IGraphStateManager):
    """
        ...
        async with self._state_lock:
            ...
            try:
                ...
                if current_node_state.status != old_status:
                    ...
                    raise ValueError(
                        f"Concurrent modification detected: expected status {old_status}, found {current_node_state.status}. Another transaction modified this node."
                    )
                ...
                raise ValueError(f"Invalid transition from {old_status} to {new_status}: {str(e)}") from e
    ...
    async def add_dependency_edge(self, from_id: str, to_id: str) -> None:
        ...
        async with self._state_lock:
            ...
            await self.event_publisher.emit_dependency_added(from_task_id=from_id, to_task_id=to_id)
    ...
    def get_execution_statistics(self) -> dict[str, Any]:
        ...
        return {"total_nodes": len(all_nodes), "version": self.version, **status_counts}
