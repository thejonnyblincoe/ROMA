...
class CircuitBreakerConfig(BaseModel):
    ...
    success_threshold: int = Field(
        default=2, description="Successes needed to close circuit from half-open"
    )
...
class CircuitBreaker(BaseModel):
    ...
    state: CircuitBreakerState = Field(
        default=CircuitBreakerState.CLOSED, description="Current circuit state"
    )
    last_failure_time: datetime | None = Field(default=None, description="Last failure timestamp")
...
class RecoveryManager(BaseModel, IRecoveryManager):
    """
    ...
    permanent_failures: set[str] = Field(
        default_factory=set, description="Permanently failed task IDs"
    )
    ...
    async def handle_failure(
        self, failed_node: TaskNode, error: Exception, _context: dict | None = None
    ) -> RecoveryResult:
        """
        ...
        # Retries exhausted - decide next action based on error type
        return await self._escalate_failure(failed_node, error)

    async def _escalate_failure(
        self, failed_node: TaskNode, error: Exception
    ) -> RecoveryResult:
        """
        ...
        if failed_node.is_composite:
            ...
            updated_node = failed_node.model_copy(
                update={"retry_count": 0, "version": failed_node.version + 1}
            )
    ...
    def evaluate_terminal_children(
        ...
        if not child_nodes:
            logger.debug(
                f"Parent {parent_node.task_id} has no children, proceeding with aggregation"
            )
