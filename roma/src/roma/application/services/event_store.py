...
from ...domain.events.task_events import BaseTaskEvent, TaskEvent
    ...
    def _apply_filter(
        ...
        if event_filter.event_type:
            filtered_events = [
                ...
                if e.event_type == event_filter.event_type
            ]
        
        if event_filter.start_time:
            filtered_events = [
                e for e in filtered_events
                if e.timestamp >= event_filter.start_time
            ]
        
        if event_filter.end_time:
            filtered_events = [
                e for e in filtered_events
                if e.timestamp <= event_filter.end_time
            ]
...
_global_event_store: Optional[InMemoryEventStore] = None
