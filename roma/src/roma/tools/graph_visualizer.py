...
class GraphVisualizer:
    ...
    def render_ascii(self, show_details: bool = False) -> str:
        """
        ...
        root_nodes = [node for node in self.graph.get_all_nodes() if node.parent_id is None]
    ...
    def _render_node_ascii(
        self, node: TaskNode, prefix: str, is_last: bool, show_details: bool
    ) -> list[str]:
        ...
        # Node type symbol
        type_symbol = {TaskType.THINK: "🧠", TaskType.WRITE: "✏️", TaskType.RETRIEVE: "📥"}.get(
            node.task_type, ""
        )
        ...
        for i, child in enumerate(children):
            ...
            lines.extend(self._render_node_ascii(child, child_prefix, is_child_last, show_details))
    ...
    def _add_nodes_to_dot(self, dot: Any, include_metadata: bool) -> None:
        ...
        for node in self.graph.get_all_nodes():
            ...
            dot.node(node.task_id, label=label, fillcolor=color, style="filled", shape=shape)
        ...
        for task_type in TaskType:
            ...
            with dot.subgraph(name=f"cluster_{task_type.value}") as cluster:
                ...
                for node in type_nodes:
                    ...
                    cluster.node(
                        node.task_id, label=label, fillcolor=color, style="filled", shape=shape
                    )
    ...
    def render_matplotlib(
        ...
        for status in TaskStatus:
            status_nodes = [
                node.task_id for node in self.graph.get_all_nodes() if node.status == status
            ]
            if status_nodes:
        ...
        nx.draw_networkx_edges(
            self.graph._graph, pos, edge_color="gray", arrows=True, arrowsize=20, ax=ax
        )
        ...
        nx.draw_networkx_labels(self.graph._graph, pos, labels, font_size=8, ax=ax)
    ...
    def export_summary(self, output_path: str) -> str:
        ...
        summary_lines.extend(["", "GRAPH STRUCTURE", "-" * 20, ascii_art])
...
# Utility functions
def visualize_graph(graph: DynamicTaskGraph, method: str = "ascii", **kwargs) -> str:
    """
...
async def interactive_graph_monitor(
    graph: DynamicTaskGraph, refresh_interval: float = 1.0, max_iterations: int = 100
) -> None:
    """
    ...
    for i in range(max_iterations):
        # Clear console (async-safe alternative)
