...
class TaskNode(BaseModel):
    ...
    started_at: datetime | None = Field(default=None, description="Start execution timestamp")
    ...
    def transition_to(self, status: TaskStatus, **updates: Any) -> "TaskNode":
        ...
        if status == TaskStatus.EXECUTING and not self.started_at:
            timestamp_updates["started_at"] = datetime.now(UTC)
        elif status.is_terminal and not self.completed_at:
            timestamp_updates["completed_at"] = datetime.now(UTC)
    ...
    def with_result(self, result: Any, metadata: dict[str, Any] | None = None) -> "TaskNode":
        """
    ...
    def with_error(self, error: str, metadata: dict[str, Any] | None = None) -> "TaskNode":
        """
    ...
    def add_child(self, child_id: str) -> "TaskNode":
        ...
        return self.model_copy(
            update={"children": self.children | {child_id}, "version": self.version + 1}
        )
    ...
    def update_metadata(self, **metadata: Any) -> "TaskNode":
        ...
        """
        return self.model_copy(
            update={"metadata": {**self.metadata, **metadata}, "version": self.version + 1}
        )
    ...
    def set_node_type(self, node_type: NodeType) -> "TaskNode":
        ...
        return self.model_copy(update={"node_type": node_type, "version": self.version + 1})
    ...
    def increment_retry(self) -> "TaskNode":
        ...
        return self.model_copy(
            update={"retry_count": self.retry_count + 1, "version": self.version + 1}
        )
