...
Simple video artifact that wraps MediaFile for video content.
"""
...
class VideoArtifact(BaseArtifact):
    ...
    Simple wrapper around MediaFile for video files.
    MediaFile already handles all video formats and sources.
    """
    ...
    def media_type(self) -> MediaType:
        """Get the media type for video artifacts."""
    ...
    def from_path(
        ...
        """Create VideoArtifact from local file path."""
    ...
    def from_bytes(
        ...
        """Create VideoArtifact from raw bytes."""
    ...
    async def get_content(self) -> Optional[bytes]:
        """Get the raw video content as bytes."""
    ...
    def get_content_summary(self) -> str:
        ...
        resolution_str = f" {self.width}x{self.height}" if self.width and self.height else ""
        fps_str = f" @{self.fps}fps" if self.fps else ""
        return f"Video: {self.media_file.get_content_summary()}{duration_str}{resolution_str}{fps_str}"
    ...
    def get_file_extension(self) -> Optional[str]:
        """Get file extension from the video file."""
