...
class DynamicTaskGraph(BaseModel):
    ...
    root_node_id: str | None = Field(default=None, description="Root node task ID")
    ...
    def __init__(self, root_node: TaskNode | None = None, **data: Any) -> None:
    ...
    async def update_node_metadata(
        ...
        async with self._lock:
            ...
            updated_node = current_node.model_copy(
                update={"metadata": updated_metadata, "version": current_node.version + 1}
            )
