...
class ToolkitConfig:
    ...
    implementation: Optional[str] = None  # For custom toolkits
...
class AgentToolkitsConfig:
    ...
    """
    toolkits: List[Union[str, ToolkitConfig]] = Field(default_factory=list)
    available_tools: List[str] = Field(default_factory=list)  # Alternative format
    
    @field_validator("toolkits")
    @classmethod
    def validate_toolkits(cls, v: List[Union[str, ToolkitConfig]]) -> List[Union[str, ToolkitConfig]]:
