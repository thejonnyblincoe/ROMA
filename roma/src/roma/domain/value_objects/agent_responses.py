...
class PlannerResult(BaseModel):
    ...
    estimated_total_effort: int | None = Field(
        default=None,
        ge=1,
    ...
    def get_examples(cls) -> list[dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "confidence": 0.91,
                "sources_used": [
                    "adoption_research_task",
                    "policy_research_task",
                    "technology_research_task",
                ],
                "gaps_identified": [
                    "regional_regulatory_variations",
                    "grid_integration_challenges",
                ],
                "quality_score": 0.88,
            },
            {
...
class PlanModifierResult(BaseModel):
    ...
    new_dependencies: dict[str, list[str]] | None = Field(
        default=None,
        ...
        return [
            {
                "modified_subtasks": [
                    ...
                    },
                    {
                        ...
                        "metadata": {
                            "time_period": "2_years",
                            "metrics": ["revenue", "profit_margin", "deliveries"],
                        },
                    },
                ],
                "changes_made": [
                ...
                "new_dependencies": {"competitive_analysis_task": ["market_share_task"]},
            },
            {
                "modified_subtasks": [
                    {
...
def create_atomic_result(reasoning: str, confidence: float = 1.0) -> AtomizerResult:
    ...
    return AtomizerResult(is_atomic=True, reasoning=reasoning, confidence=confidence)


def create_composite_result(reasoning: str, confidence: float = 1.0) -> AtomizerResult:
    """Create a composite (PLAN) atomizer result."""
    return AtomizerResult(is_atomic=False, reasoning=reasoning, confidence=confidence)


def create_successful_execution(result: Any, sources: list[str] | None = None) -> ExecutorResult:
    """Create a successful executor result."""
    return ExecutorResult(result=result, sources=sources or [], success=True)


def create_failed_execution(error_msg: str, metadata: dict[str, Any] | None = None) -> ExecutorResult:
    """Create a failed executor result."""
    return ExecutorResult(result=error_msg, metadata=metadata or {}, success=False, confidence=0.0)
