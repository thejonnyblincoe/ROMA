...
from dataclasses import field, dataclass as dataclass_decorator
...
class AgentMappingConfig:
    ...
    def __post_init__(self):
        ...
        valid_task_types = [task_type.value for task_type in TaskType] + ["AGGREGATE", "default", "error_recovery"]
                ...
                for key, agent_data in mapping.items():
                    ...
                    if isinstance(agent_data, (dict, DictConfig)) and "_target_" in agent_data:
                        try:
                            ...
                            raise ValueError(f"Failed to instantiate agent {agent_type}.{key}: {e}")
                    else:
    ...
    def get_agent_for_task(self, agent_type: str, task_type: TaskType) -> Optional[AgentConfig]:
